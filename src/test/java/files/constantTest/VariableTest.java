...
public class VariableTest {
    ...
    @Test
    void operationTest() {
        ConstantNum one = c.cNum(1);
        ConstantNum two = c.cNum(2);
        Operations add = new Mul(one, two);
        String name = "name";
        ConstantNum num = v.variable(name, 1);
        assertEquals(one, num);
        Operations op = v.variable(name, add);
        assertEquals(two, op.eval());
